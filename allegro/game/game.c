    ...
    for(int i = 0; i < ALIENS_N; i++)
    {
        if(!aliens[i].used)
        {
            ...
            if(new_quota > 0)
            {
                ...
                aliens[i].type = (ALIEN_TYPE) between(0, ALIEN_TYPE_N - 1);
        ...
        if(aliens[i].life <= 0)
        {
            ...
            switch(aliens[i].type)
            {
                ...
                default:
                    // Você pode adicionar algum tratamento aqui, ou deixar em branco se não precisar
                    break;
            }
            aliens[i].used = false;
            continue;
